...
// user table with default users

// scales routes
app.get("/api/scales", (req, res, next) => {
...
app.get("/api/scale/:scale_id", (req, res, next) => {
...
app.get("/api/measurements/:scale_id", (req, res, next) => {
...
app.get("/api/measurements/:grid_name", (req, res, next) => {
  let sql = `SELECT scale_id, grid_id, grid_name, operator_name, measurement_time, measurement_date, weight FROM Measurements WHERE grid_name = ?`;
  let params = [req.params.grid_name];
  db.all(sql, params, (err, rows) => {
    if (err) return res.status(400).json({ error: err.message });
    res.json({
      message: "success",
