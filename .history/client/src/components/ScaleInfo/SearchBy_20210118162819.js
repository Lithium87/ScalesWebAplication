...
class SearchBy extends Component {
  ...
  filterResults = (event, data) => {
    ...
    if (fromDate) {
      data.filter((row) => row.measurement_date === fromDate);
      console.log("Filtered");
    }

    if (toDate) {
      data.filter((row) => row.measurement_date === toDate);
      console.log("Filtered");
    }

    if (this.state.materialValue) {
      data.filter((row) => row.grid_name === this.state.materialValue);
      console.log("Filtered");
    }

    if (this.state.operatorValue) {
      data.filter((row) => row.operator_name === this.state.operatorValue);
      console.log("Filtered");
    }
